...
;(function () {
  this.jCircle = function () {
    ...
    if (this.options.mainViewStyle == 'enlarge-view') {
      this.options.mainContent.className = this.options.mainContent.className
          + ' enlarge-view';
    }
    ...
    if (this.options.animateCircles) {
      ...
      for (var a in animateTypes) {
        if (this.options.animateType == animateTypes[a]) {
          var animationRepeat;
          animationRepeat = setInterval(animate, _.options.speed * 1000, this,
              animateTypes[a]);
          if (_.options.stopOnOverMini) {
            for (var i = 0; i < this.options.circleNumber; i++) {
              this.options.minCircles[i].addEventListener('mouseover',
                  function () {
                    _.options.animateStatus = 'pause';
                  });
              this.options.minCircles[i].addEventListener('mouseout',
                  function () {
  ...
  function init() {
    ...
    if (circleNumber > 0) {
      ...
      container.style.height = parseFloat(getStyle(circle, 'width')) + 200
          + 'px';
      ...
      for (var i = 0; i < circleNumber; i++) {
        ...
        mCircle.getElementsByTagName('a')[0].addEventListener('click',
            function (e) {
              e.preventDefault();
              ...
              if (contentType == 'images') {
                ...
                if (this.parentNode.getElementsByTagName('div')[0]) {
                  caption = this.parentNode.getElementsByTagName(
                      'div')[0].innerHTML;
                }
      ...
      if (this.options.contentType == 'images') {
        mainContent.getElementsByTagName('img')[0].setAttribute('src',
            circle.getElementsByTagName('div')[0].getElementsByTagName(
                'a')[0].getAttribute('href'));
        if (circle.getElementsByTagName('div')[0].getElementsByTagName(
            'div')[0]) {
          caption.innerHTML = circle.getElementsByTagName(
              'div')[0].getElementsByTagName('div')[0].innerHTML;
        }
      } else if (this.options.contentType == 'text') {
        mainContent.innerHTML = circle.getElementsByTagName(
            'div')[0].getElementsByTagName('div')[0].innerHTML;
      }
      return _;
    } else {
  ...
  function animate(elem, animateType) {
    if (!elem.options.animateCircles || elem.options.animateStatus
        == 'pause') {
      return;
    }
    ...
    switch (animateType) {
      case 'rotateAround':
        ...
        for (var i = 0; i < elem.options.circleNumber; i++) {
          ...
          if (parseInt(getStyle(div[i], 'right')) <= parseInt(
              getStyle(div[topCircleNTH], 'right'))) {
            topCircleNTH = i;
          }
        }
        if (elem.options.contentType == 'images') {
          elem.options.mainContent.getElementsByTagName('img')[0].setAttribute(
              'src',
              div[topCircleNTH].getElementsByTagName('a')[0].getAttribute(
                  'href'));
          if (div[topCircleNTH].getElementsByTagName('div')[0]) {
            elem.options.mainContent.getElementsByTagName(
                'div')[0].innerHTML = div[topCircleNTH].getElementsByTagName(
                'div')[0].innerHTML;
          }
        }
        if (elem.options.contentType == 'text') {
          elem.options.mainContent.innerHTML = div[topCircleNTH].getElementsByTagName(
              'div')[0].innerHTML;
        }
        break;
      default:
