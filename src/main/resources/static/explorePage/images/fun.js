...
function scrollact(e, fix) {
  ...
  if ($(window).scrollTop() + Math.abs($(window).height() - $(e).height())
      >= +$(e).offset().top - fix) {
    return true;
  }
}
...
function autodiv(e, type) {
  $(e).each(function (index, element) {
    ...
    if (type == "c") {
      $(this).width(
          $(this).parent().width() / $(this).parent().attr("w") * $(this).attr(
          "w"));
      $(this).height(
          $(this).parent().height() / $(this).parent().attr("h") * $(this).attr(
          "h"));
      $(this).css("left",
          $(this).parent().width() / $(this).parent().attr("w") * $(this).attr(
          "l"));
...
function autoimg(e, w, h) {
  ...
  if ((w / h) < (img.width / img.height)) {
    ...
    if (img.height > h) {
      ...
      $(e).css("margin-top", (h - img.height) * 0.5);
    }
  }
}
...
function autoimg1(e) {
  ...
  if ($(e).parent().width() / $(e).parent().height() >= $(e).width() / $(
      e).height()) {
    $(e).css({width: "100%", height: "auto"});
  } else {
    $(e).css({height: "100%", width: "auto"});
...
function hover(e, classname, child) {
  ...
  if (child == undefined) {
    ...
    $(e).hover(
        function () {
          $(this).find(child).addClass(classname);
        },
        function () {
          $(this).find(child).removeClass(classname);
        }
    );
  }
}
  ...
  for (var i = 1; i < 30; i++) {
    ...
    if (document.getElementById("tab" + id + i)) {
      document.getElementById(
          "tab" + id + i).className = document.getElementById(
          "tab" + id + i).className.replace("now", "");
    }
  }
  document.getElementById("tab" + id + idx).className = document.getElementById(
      "tab" + id + idx).className + " now";
...
function showslider(id, e, cname) {
  ...
  if ($(id).height() == $(id).attr("h") && $(id).height() < $(id).find(
      ".bd").height()) {
...
function divcenter(e, mintop) {
  ...
  var top, left;
  if ($(window).height() > h) {
    top = $(window).scrollTop() + ($(window).height() - h) * 0.5;
  } else {
    top = $(window).scrollTop() + mintop;
  }
  left = w * 0.5 * -1;
