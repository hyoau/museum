...
(function ($) {
  $.fn.mScroll = function (options) {
    ...
    this.each(function () {
      //初始化
      var f, s, c, n, np, ns, w, ww, h, hh, w1, h1, size, iv, ti, sw = 1,
          idx = 0, cm = 0;
      f = $(this);

      if (f.children().is("ul") && f.children().size() > 1
          && !f.children().hasClass("child")) {
      ...
      if (opts.cycle == 1 && size > opts.minsize) {
        s.append(s.html());
      }
      c = s.children();
      ...
      n = $(opts.nav);
      if (opts.scrollnav) {
        w1 = n.width() + parseInt(n.css("margin-left")) + parseInt(
            n.css("margin-right"));
        h1 = n.height();
        np = $(opts.nav).parent();
      ...
      if (size > opts.minsize) {//判断是否满足数量
            ...
            if (opts.direction == "h") {
              if (act == "left") {
                if (opts.cycle != 1) {
                  if (idx + opts.minsize < size) {
                    idx++;
                    f.animate({scrollLeft: f.scrollLeft() + w}, opts.speed,
              ...
              if (act == "right") {
                ...
                if (opts.cycle == 1) {
                  ...
                  if (opts.cycle == 1 && f.scrollLeft() <= 0) {
                    f.scrollLeft(ww * 0.5);
                  }
                  f.animate({scrollLeft: f.scrollLeft() - w}, opts.speed,
                      opts.easing, function () {
              ...
              if (act == "left") {
                if (opts.cycle != 1) {
                  if (idx + opts.minsize < size) {
                    ...
                    f.animate({scrollTop: f.scrollTop() + h}, opts.speed,
                        opts.easing, function () {
                          sw = 1;
                        });
                  } else {
        ...
        function autonav() {
          if (opts.nav_direction == "h") {
            ...
            if (idx == 0) {
              np.stop().animate({scrollLeft: 0}, opts.speed);
            }
            if (idx == size - 1) {
              np.stop().animate({scrollLeft: size * w1}, opts.speed);
            }
          }
          if (opts.nav_direction == "v") {
            ...
            if (idx == 0) {
              np.stop().animate({scrollTop: 0}, opts.speed);
            }
            if (idx == size - 1) {
              np.stop().animate({scrollTop: size * h1}, opts.speed);
            }
          }
        }
        ...
        n.each(function (index, element) {
          $(this).click(function () {
            if (opts.direction == "h") {
              f.stop().animate({scrollLeft: index * w}, opts.speed, opts.easing,
                  function () {
                    sw = 1;
                  });
            }
            if (opts.direction == "v") {
              f.stop().animate({scrollTop: index * h}, opts.speed, opts.easing,
                  function () {
                    sw = 1;
                  });
            }
